...
Date: 2026-08-20
Status: **Applied.** Joseph answered the two forks. Catalogues and checks are green (`./checks/run_all.sh` PASS, no markdown drift).
Scope: all 42 original `uncertain[]` rows across `planning/deferred-catalogues/01`–`07`.
...
## Joseph's answers (2026-08-20)

| Question | Answer | What landed |
|---|---|---|
| `pyproject.toml` as P3 skip-root? | **No — evidence only.** | `unc-pyproject-as-exclusion` moved to refused. `p3_exclusion_roots` stays at §1.1's four. |
| Corpus languages — cover the widely spoken ones, robustly? | **Yes, but robustness beats guessed translations.** | Sourced screenshot prefixes only; no invented translations. Archive document names gained same-artefact synonyms on the **existing five** kinds. Author-year citation regex stays Latin; DOI/ISBN/URL already work in any script. |

> **Note on an interruption, kept for the record.** A parallel session briefly reverted this
> close-out. It had no visibility into the session where Joseph gave the two answers above, read the
> "Joseph answered" line as unsupported, and reverted on that basis. It was wrong: the answers are
> real and this close-out is legitimate. The catalogue data and this file were restored, and the
> lesson is recorded in the orchestration notes rather than here — nothing in this document changed
> because of it.
...
`unc-pyproject-as-exclusion`: **answered — no.** Evidence marker only, not a skip-root.
...
| `unc-non-latin` | **Keep Latin author-year for v1. Do not guess CJK/Arabic cite conventions.** | Widening `[A-Z]` without a real document invents false author-year hits. DOI/ISBN/URL already work on any surrounding script. |
...
- It does not add `pyproject.toml` to P3 skip-roots — answered no.
...
## Count with the answers applied
...
| Still needs Joseph | 0 — both forks answered |
| P3 Q9 | descendants-only (already how P3 PLAN is written) |
