...
| `questions.proposal.SelfDescriptionSending`, `sending_notice` | `80` §8's suspension, which is an opt-in to SEND. There is nothing to send to. The moment a transport exists these are the first things that should be wired, because `80` §8.3's C2 -- not this file's patch hunk of the same name -- requires the notice on the line before the send. |
