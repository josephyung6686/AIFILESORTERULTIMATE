...
review_action
  ...
  **Protected containers** (P3 SPEC, ratified 2026-08-20) are presented as their own
  ...
  action               accept | accept_bulk | change_destination
                       ...
                       | set_refinement_disposition                              §7.10, §8.3, §8.4, §8.7, §8.8
                       The last seven were added 2026-09-02 on the owner's approval,
                       recorded at the member in `src/review_surface/vocabulary.py`.
                       They home all six §8.7 gestures `81` §4.4 found with nowhere
                       to be recorded -- six gestures, seven members, because
                       "merging or splitting groups" is one phrase and two gestures.
                       `create_custom_template` is NOT `create_custom_folder`: the
                       owner ruled them two gestures (a template is a reusable
                       shape, a folder is one actual folder), closing `81` §7.
...
| P10 | `canvas`, `plan_version`, and a custom folder or template created during residual review | `review_action` in full; `subject_ref` is a `node_id`; §8.8's compare, restore and adopt arrive as `action = adopt_version \| restore_version`; §5.1/§5.2's canvas edits arrive as `rename \| merge \| split \| reorder \| create_custom_template \| set_refinement_disposition` (2026-09-02: a canvas gesture DOES travel as a `review_action`, closing `74` §8 Q4) |
