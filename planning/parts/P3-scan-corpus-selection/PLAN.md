...
def scan(conn: sqlite3.Connection, selection_id: str, *,
    ...
    for item in walk(source, sources=sources, candidate_roots=candidate_roots,
        ...
        if isinstance(item, ExclusionVerdict):
            ...
            if item.dataless:
                # 11 §5: do not materialize, hash, or extract. No `files` row and
                # no extraction run — P3's duty is unchanged by C4. What changed is
                # that the state now HAS a name: P4 `completeness: dataless`
                # (OQ6 closed 2026-08-20). Writing that run is the extraction
                # caller's job, never P3's; P3 records the detection and stops.
