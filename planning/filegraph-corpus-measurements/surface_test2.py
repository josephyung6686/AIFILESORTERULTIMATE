...
VER = re.compile(r'\s*(\(\d+\)|copy( \d+)?|[ _-]v\d+|final\d*|updated?|draft\d*)\s*$', re.I)
def stem_key(n):
...
# facts do NOT union — each fact is its own candidate group
...
groups=[(v,"version family") for v in comp.values() if len(v)>=4]
groups+= [([Path(x) for x in sorted(s)], f"share {e[0]}={e[1]}") for e,s in FACT.items() if len(s)>=4]
