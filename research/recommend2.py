...
# NEVER a destination: tool-generated / dependency / system trees
SKIP_DIR = {"node_modules","__pycache__","venv",".venv","env","build","dist",".next",
            "target","vendor","Pods","DerivedData","Library","site-packages","bower_components",
            ".git",".svn","cache","Cache","tmp","temp",".cache","out","bin","obj"}
SKIP_MARK = {"package.json","requirements.txt","Cargo.toml","go.mod",".git","pubspec.yaml"}
...
for root in ROOTS:
    ...
    for d in root.rglob("*"):
        try:
            ...
            parts = d.relative_to(HOME).parts
            if any(p.startswith(".") for p in parts): continue
            if any(p in SKIP_DIR for p in parts): continue            # FIX 1: no node_modules
            if "Auto-Save" in d.name or "Previews" in d.name: continue
            rp = d.relative_to(root).parts
            if len(rp) > 3: continue
            # inside a code project? skip the whole subtree as a destination
            if any((d.parents[i] / m).exists() for i in range(min(3, len(d.parents)))
                   for m in ("package.json", "requirements.txt", "Cargo.toml")): continue
...
# FIX 2: identity/ubiquity stoplist derived from the corpus itself
_df = Counter()
for f in loose: _df.update(toks(f.stem))
UBIQ = {t for t, c in _df.items() if c > 0.04 * len(loose)}
print(f"auto-stoplisted {len(UBIQ)} ubiquitous tokens (not categories): "
      f"{sorted(UBIQ)[:10]}\n", flush=True)
...
for f in unmatched:
    for t in (toks(f.stem) - UBIQ): tokgroups[t].append(f)
