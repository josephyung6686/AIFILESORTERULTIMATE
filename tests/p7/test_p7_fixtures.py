...
def test_the_ten_required_keywords_have_no_default_and_the_two_optional_ones_do():
    ...
    # TWO optional, again. The suspension was briefly a thirteenth and is now in
    # the stored `Policy` instead: §8.4's audit names the authorizing policy, and a
    # constructor argument reaches no record, so the one act that cannot be taken
    # back was authorised by something an auditor could not name.
