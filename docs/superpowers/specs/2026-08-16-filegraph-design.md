...
### Role belongs in metadata, not in the tree — confirmed by shipping standards

The **TMF Reference Model** (clinical trials, 250 artifact types, machine-parsed) has this exact
problem at scale:

```
sponsor-only        137 artifacts
BOTH parties        109 artifacts   ← 44% belong to two role-bearing parties at once
```

**It does not fork the tree. It tags.** Role is an orthogonal boolean axis, alongside an explicit
`Artifact Owner` field defined as *"the person or department or vendor that creates and maintains
a given artifact, **regardless of its location**"* — authorship deliberately decoupled from
placement. Forking on party would duplicate 44% of the taxonomy.

**So the correction to the correction:** `addressed_to` is a folder dimension (high information);
`authored_by` is a **metadata tag that never becomes a folder level** (near-constant, low
information). Splitting into two *dimensions* was half the fix.

**The hard constraint, taken verbatim from TMF Zone 9:**

> *"this zone concerns the establishment and management of third party relationship and is **not
> intended as the filing location for artifacts that are generated by a third party**. These
> artifacts should be filed in the zone that is relevant."*

**An entity dimension exists to manage the relationship with that entity — not to collect
everything that entity touched.** This goes in the template validator as a hard check: no
folder dimension may be filled by "who produced this."

**When role must be a folder level, name it and pair it.** Law firm practice, unanimous across
five published matter templates:

```
Discovery/Request and Response – Us to Them   ·   …– Them to Us
Discovery/Records from Client                 ·   Records from Opposing Party
Correspondence/Opposing Counsel · Client · Court · Third Parties
```

A bare `Columbia/` folder is exactly what both traditions avoid.
...
**Confirmed by seven independent published file plans**, and it is the dominant real-world order:

```
function → sub-function/activity → record-type → time
```

Lancaster University, UBC Records Management, Victoria University/Toronto, U Washington, NARA,
and practising lawyers all converge on it. **Zero records-management sources place year above
function.**

**The one sharp exception, which becomes a routing rule:** capture-centric media put **time
first**, because time *is* the subject — Lightroom's default import is `YYYY/YYYY-MM-DD`, Google
Takeout uses `Photos from YYYY/`.

> **Documents and records → time last. Photos, captures and events → time first.**

**Function beats subject, and beats organisational unit.** Manitoba's records office states the
reason: *"Although subjects and organizational structures may change, the core activities
generally stay the same over time."* A project or function folder survives; a topic label drifts.

**Put the date in the filename even when it is also a folder** — unanimous across every source.
The stated rationale is that file metadata is not trustworthy: *"The date and time stamp on a
file is changed when we scan documents, run OCR, separate files into smaller pieces."*
...
| Check | Limit | Why |
|---|---|---|
| Depth | **≤ 4 dimensions** | 82% of real retrievals at depth ≤4; measured across 157 real published folders: **80% at ≤3, 95.5% at ≤4**, max 5 |
| Branches per level | **≥ 3, target 7–12** | Fewer earns nothing. Upper target from convergent practice: records-management guidance says 7–12 top-level functions, law templates run 8–11, Johnny.Decimal hard-caps at 10 |
| Files per leaf | **≤ ~21** | Measured: **100%** of folders in a real published tree have ≤21 children. Safe ceiling, ~2× typical practice |
| **No redundant level** | reject | Lancaster's own anti-pattern rule: `Contract Management / Contracts of Employment / Contracts` — *"Redundant folders often repeat the information of a folder from a level above"* |
| **No entity-as-collector** | reject | TMF Zone 9: a dimension may not be filled by "who produced this" |

Depth 5+ unlocks only when a template declares genuinely extra axes — the one domain that
legitimately exceeds 4 is multi-party capital projects (US federal AEC runs 6–7), because it
carries *site*, *building* and *submittal-snapshot* dimensions that don't exist elsewhere.

### Escape hatches are per-branch, never global

The TMF ships a uniform `General` section under **all 11 zones**, containing the same catch-all
leaves — `Relevant Communications` (11×), `Tracking Information` (11×), `Filenote` (11×),
`Meeting Material` (10×).

**That is the disciplined answer to "where does the miscellaneous go": a named escape hatch on
every branch, not one global `Unsorted/`.** A file that clearly belongs to `Columbia/2026-Spring/`
but has no determinable work-type lands in `Columbia/2026-Spring/General/` — which is findable,
scoped, and honest. A global `Unsorted/` is where files go to be lost.

This also gives shallow placement (the chosen behaviour for low confidence) a real destination
